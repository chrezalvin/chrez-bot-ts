const debug = require("debug")("ChrezBot:playlist");

import { ErrorValidation, MyEmbedBuilder } from "@library";
import { ChatInputCommandInteraction, InteractionReplyOptions, Message, MessageCreateOptions, VoiceBasedChannel } from "discord.js";
import z from "zod";
...
export const playlistSchema = z.object({
    ...
    options: z.enum(["view", "save", "remove", "update", "play"]),
    playlistName: z.string().optional(),
    voiceChannel: z.custom<VoiceBasedChannel>().nullable(),
    requester: z.object({
        name: z.string(),
        iconUrl: z.string()
    })
...
export type I_Playlist = z.infer<typeof playlistSchema>;

export async function playlist(args: I_Playlist): Promise<MessageCreateOptions & InteractionReplyOptions>{
    return {
        content: "not implemented!"
    };
    // TODO
    // if(!args.message.member?.user.id)
    //     throw new ErrorValidation("something_not_found", "user");

    // const userId = args.message.member.user.id;
    // const discordYtPlayer = createDiscordYtPlayerIfNotExist(args.message.guild?.id ?? "");
    // const discordEmbed = new MyEmbedBuilder();

    // if(args.options === "save"){
    //     if(discordYtPlayer.queue.length === 0)
    //         throw new ErrorValidation("something_not_found", "queue");

    //     if(!args.playlistName)
    //         throw new ErrorValidation("something_not_found", "playlist name");
    ...
    //     if(!playlist)
    //         throw new ErrorValidation("something_not_found", `playlist with name ${args.playlistName}`);
