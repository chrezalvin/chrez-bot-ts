...
import { UserPreferedFoodBuff, UserPreferedFoodBuffCreate } from "../../types/models/UserPreferredFoodBuff";
...
export async function createUserPreferedFoodBuff(schema: UserPreferedFoodBuffCreate): Promise<UserPreferedFoodBuff[]>{
    ...
    const create = schema.food_buffs.map(food_buff => ({food_buff, discord_user: schema.discord_user}));
