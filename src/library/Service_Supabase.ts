 ...
 */
export class Service_Supabase<T>{
    public static s_services: Service_Supabase<any>[] = [];
    ...
    constructor(settings: {
        ...
        this.m_tableName = settings.tableName;

        // fills the cache when created
        this.getAllData(true);

        // put the instance into the static array
        Service_Supabase.s_services.push(this);
    }
    ...
    //     const data = await supabase.from(this.m_tableName).select("*").eq("id", id);

    //     const docRef = doc(Service_Supabase.db, this.m_tableName, id);
     ...
     */
    protected async _getAllData(): Promise<T[] | null>{
        ...
        if(data.length !== 0){
            // if typeguard isn't set, assumes that the type is already T[]
            if(this.m_typeGuard)
                return data.filter(ele => this.m_typeGuard!(ele)) as T[];
            else
                return data as T[];
     ...
     */
    protected async _addData(data: T): Promise<T>{
        const res = await supabase.from(this.m_tableName).insert(data);
    ...
    protected async _deleteData(id: string): Promise<void>{
        await supabase.from(this.m_tableName).delete().eq("id", id);
    ...
    protected async _batchDelete(ids: string[]): Promise<void>{
        const batch = writeBatch(Service_Supabase.db);
    ...
    public async getById(id: string): Promise<T>{
        debug(`gets data from ${this.m_tableName} with id: ${id}`);
    ...
    public async getAllData(fetch?: boolean): Promise<Map<string, T>>{
        debug(`gets all data from ${this.m_tableName}`);
    ...
    public async updateData(id: string, data: T): Promise<void>{
        debug(`updates data in ${this.m_tableName} with id: ${id}`);
    ...
    public async addData(data: T, id?: string): Promise<string>{
        ...
        debug(`added new data to ${this.m_tableName} with id: ${resId}`);
    ...
    public async deleteData(id: string): Promise<void>{
        debug(`deletes data from ${this.m_tableName} with id: ${id}`);
    ...
    public async batchDelete(ids: string[]): Promise<void>{
        debug(`deletes data from ${this.m_tableName} with ids: ${ids}`);
