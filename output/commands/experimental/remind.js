...
function run(args) {
    if (args?.description !== undefined && args?.description !== "") {
        const embed = new _library_1.MyEmbedBuilder({ title: args.description, description: "agrees[rngInt(0, agrees.length - 1)]" });
        return { embeds: [embed] };
    }
    else
...
const slashCommand = new discord_js_1.SlashCommandBuilder()
    ...
    .setRequired(false)
    .setName("mention"));
const remind = new _library_1.CommandBuilder()
    .setName("remind")
