...
function run(args) {
    if (args?.description !== undefined && args?.description !== "") {
        const embed = new basicFunctions_1.MyEmbedBuilder({ title: args.description, description: agrees_json_1.agrees[(0, basicFunctions_1.rngInt)(0, agrees_json_1.agrees.length - 1)] });
        return { embeds: [embed] };
    }
    else
        return agrees_json_1.agrees[(0, basicFunctions_1.rngInt)(0, agrees_json_1.agrees.length - 1)];
}
...
const agree = new CommandBuilder_1.CommandBuilder()
    .setName("agree")
