...
const score = new _library_1.Score();
const run = (args) => {
    let choice = args.choice;
    let botChoice = roshambo[(0, _library_1.rngInt)(0, roshambo.length - 1)];
    ...
    else if ((0, _library_1.rngInt)(0, 100) % 5 === 0 && score.winRate > 0.9)
        footerMessage = `i have ${Math.floor(score.winRate * 100)}% win rate, you can't defeat me`;
    else if ((0, _library_1.rngInt)(0, 100) % 5 === 0 && score.winRate > 0.7)
...
const slashCommand = new discord_js_1.SlashCommandBuilder().setName("roshambo")
...
const chrezRoshambo = new _library_1.CommandBuilder()
    .setName("roshambo")
    .setAlias(["rps"])
    .setDescription("plays rock paper scissor")
    .setExamples([
    { command: `${_config_1.prefixes[0]} roshambo rock`, description: "" }
])
    .setSlash({
    slashCommand,
