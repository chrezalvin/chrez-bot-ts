...
exports.FileManagerSupabase = void 0;
const _config_1 = require("../config");
const crypto_1 = require("crypto");
class FileManagerSupabase {
    ...
    translateToUrl(fileName) {
        return _config_1.supabase.storage.from(this.m_bucket).getPublicUrl(`${this.m_path}/${fileName}`).data.publicUrl;
    }
    /**
    ...
    async uploadImage(imgUrl, filename) {
        ...
        const uploadFileRes = await _config_1.supabase
            .storage
            .from(this.m_bucket)
            .upload(`${this.m_path}/${filename ?? (0, crypto_1.randomUUID)()}.${fileExtension}`, blob);
    ...
    async deleteImage(fileName) {
        const deleteRes = await _config_1.supabase
            .storage
            .from(this.m_bucket)
     ...
     */
    async getAllFiles() {
        const allFiles = await _config_1.supabase.storage.from(this.m_bucket).list(this.m_path);
        this.m_cache = allFiles.data?.map((file) => file.name) ?? [];
        return this.m_cache.map(path => this.translateToUrl(path));
    }
}
exports.FileManagerSupabase = FileManagerSupabase;
FileManagerSupabase.s_fileManagers = [];
exports.default = FileManagerSupabase;
//# sourceMappingURL=FileManagerSupabase.js.map